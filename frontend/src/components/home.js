...
import { createToDo, getAllToDo, ToDobyIDandUpdate, ToDobyIDandDel } from '../actions/todoActions';
...
const Home = () => {
    ...
    const handleSubmit = () => {
        ...
        createToDo(body)
            .then((res) => {
                ...
                fetchTodo(); // Fetch todos after creating a new one
            })
            .catch((err) => {
                console.error(err);
            });
    ...
    const handleUpdate = async () => {
        ...
        const res = await ToDobyIDandUpdate(data.id, body)
        console.log('response', res)
    ...
    const handleDelete = async (id) => {
        const res = await ToDobyIDandDel(id)
